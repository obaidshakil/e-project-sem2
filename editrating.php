...
if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['editid'])) {
    $editid = $_GET['editid'];
    $sql = "SELECT * FROM ratings WHERE rating_id = '$editid'";
    $result = mysqli_query($conn, $sql);
    if($row = mysqli_num_rows($result)){
        $row = mysqli_fetch_assoc($result);
        $user = $row['user_id'];
        $music = $row['content_id'];
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_btn'])) {
    ...
    if (empty($ratingerror) && empty($reviewerror) && empty($usererror) && empty($musicerror) && empty($content_typeerror)) {
        $sql = "UPDATE ratings SET user_id='$user', content_id='$music', content_type='$content_type', rating='$rating', review='$review' WHERE rating_id='$editid'";
...
<body class="bg-light">
    ...
    <div class="row form-group mb-3">
        ...
        <div class="col-md-9">
            <select name="content_type" id="content_type" class="form-control">
                <option value="music" <?php if($content_type == 'music') echo 'selected'; ?>>Music</option>
                <option value="video" <?php if($content_type == 'video') echo 'selected'; ?>>Video</option>
            </select>
        </div>
    </div>
    ...
    <div class="row form-group mb-3">
        ...
        <div class="col-md-9">
            <input type="number" id="rating" name="rating" min="1" max="5" placeholder="Enter your Rating" class="form-control" value="<?php echo $rating; ?>">
    ...
    <div class="row form-group mb-3">
        ...
        <div class="col-md-9">
            <textarea id="review" name="review" placeholder="Enter your Review" class="form-control"><?php echo $review; ?></textarea>
